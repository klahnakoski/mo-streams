...
def stream(value):
    if isinstance(value, (dict, Data)):
        ...
        if not kv:
            return EmptyStream()
        _, example = kv
        return ObjectStream(((v, {"key": k}) for k, v in value.items()), example, type(example), {"key": ""}, JxType(key=JX_TEXT))
        ...
        example = first(value)
        return ObjectStream(iter(value), example, type(example), {}, JxType())
    else:
        return ObjectStream(iter([value]), value, type(value), {}, JxType())
